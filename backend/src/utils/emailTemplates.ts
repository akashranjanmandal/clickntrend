// Helper: render variant badges for a single item
const renderVariantBadge = (item: any): string => {
  const parts: string[] = [];
  if (item.color_name) parts.push(`🎨 ${item.color_name}`);
  if (item.size_name) parts.push(`📐 ${item.size_name}${item.size_code ? ` (${item.size_code})` : ''}`);
  return parts.length > 0
    ? `<br><span style="font-size:11px;color:#888;">${parts.join(' &nbsp;|&nbsp; ')}</span>`
    : '';
};

// Helper: render combo sub-products rows
const renderComboSubItems = (comboProducts: any[]): string => {
  if (!comboProducts || comboProducts.length === 0) return '';
  return comboProducts.map(cp => {
    const variantText = renderVariantBadge(cp);
    return `<tr>
      <td style="padding:6px 10px 6px 24px;border-bottom:1px solid #f5f5f5;color:#555;font-size:13px;">
        &nbsp;&nbsp;↳ ${cp.name}${variantText}
      </td>
      <td style="padding:6px 10px;border-bottom:1px solid #f5f5f5;text-align:center;font-size:13px;">${cp.quantity}</td>
      <td style="padding:6px 10px;border-bottom:1px solid #f5f5f5;text-align:right;font-size:13px;">₹${cp.price}</td>
      <td style="padding:6px 10px;border-bottom:1px solid #f5f5f5;text-align:right;font-size:13px;">₹${cp.price * cp.quantity}</td>
    </tr>`;
  }).join('');
};
...
export const getOrderConfirmationEmailHTML = (orderData: any) => {
  const itemsList = orderData.items.map((item: any) => {
    const variantText = renderVariantBadge(item);
    const isCombo = item.type === 'combo' || (item.combo_products && item.combo_products.length > 0);
    const subItemsHTML = isCombo ? renderComboSubItems(item.combo_products || []) : '';
    return `
    <tr>
      <td style="padding: 10px; border-bottom: 1px solid #eee; font-weight:${isCombo ? '600' : 'normal'};">
        ${isCombo ? '🎁 ' : ''}${item.name}${variantText}
      </td>
...
export const getOrderConfirmationEmailText = (orderData: any) => {
  const itemsList = orderData.items.map((item: any) => {
    ...
    if (item.color_name) variantParts.push(`Color: ${item.color_name}`);
    if (item.size_name) variantParts.push(`Size: ${item.size_name}${item.size_code ? ` (${item.size_code})` : ''}`);
    const variantText = variantParts.length > 0 ? ` [${variantParts.join(', ')}]` : '';
    const isCombo = item.type === 'combo' || (item.combo_products && item.combo_products.length > 0);
    let line = `- ${isCombo ? '[COMBO] ' : ''}${item.name}${variantText} x${item.quantity}: ₹${item.price * item.quantity}`;
    if (isCombo && item.combo_products && item.combo_products.length > 0) {
      const subLines = item.combo_products.map((cp: any) => {
        const cpVariants: string[] = [];
        if (cp.color_name) cpVariants.push(`Color: ${cp.color_name}`);
        if (cp.size_name) cpVariants.push(`Size: ${cp.size_name}${cp.size_code ? ` (${cp.size_code})` : ''}`);
        const cpVariantText = cpVariants.length > 0 ? ` [${cpVariants.join(', ')}]` : '';
        return `    ↳ ${cp.name}${cpVariantText} x${cp.quantity}: ₹${cp.price * cp.quantity}`;
      }).join('\n');
      line += '\n' + subLines;
    }
    return line;
