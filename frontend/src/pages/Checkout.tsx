...
import { Wallet, Truck, Tag, CheckCircle,Ruler, XCircle, ArrowLeft, ShoppingBag, Gift, AlertCircle, Sparkles } from 'lucide-react';
...
const Checkout: React.FC = () => {
  ...
  const validateCoupon = async () => {
    ...
    // Block coupons if ALL items are combos
    const hasOnlyComboItems = items.length > 0 && items.every(item => item.type === 'combo');
    if (hasOnlyComboItems) {
      setCouponError('Coupons cannot be applied to combo orders');
      return;
    }
    ...
    try {
      // Collect all category names from non-combo cart items
      const cartCategories: string[] = [];
      items.forEach(item => {
        if (item.type === 'combo') return; // skip combos
        if ((item as any).categories) {
          (item as any).categories.forEach((c: any) => {
            if (c.name && !cartCategories.includes(c.name)) cartCategories.push(c.name);
          });
        } else if ((item as any).category && !cartCategories.includes((item as any).category)) {
          cartCategories.push((item as any).category);
        }
      });

      // Subtotal of non-combo items only for coupon validation
        ...
        <div className="grid lg:grid-cols-2 gap-6 md:gap-8">
          ...
          <div className="order-1 lg:order-1">
            <div className="bg-white rounded-xl md:rounded-2xl shadow-lg p-4 md:p-6 sticky top-20">
              ...
              {/* Items List with Color and Size Info */}
              ...
              <div className="space-y-3 md:space-y-4">
                ...
                <div className="border-t pt-3 md:pt-4 mt-3 md:mt-4">
                  ...
                  {appliedCoupon ? (
                    ...
                    <>
                      {items.every(item => item.type === 'combo') ? (
                        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm text-amber-700">
                          🚫 Coupons are not applicable on combo orders
                        </div>
                      ) : (
                        <>
